...
def fetchEmbeddings(cam):
    ...
    verification_models = models.deepface_verification_models
    embeddings_dict = {}
    result = {
        "status": False,
        "embeddings": {},
...
def verify(source_embeddings, cam):
    ...
    try:
        while iter_count < 5:
            ...
            try:
                start_time = time.perf_counter()
                result1 = findMatch(image, source_embeddings[verification_models[0]["model"]], verification_models[0])
                result2 = findMatch(image, source_embeddings[verification_models[1]["model"]], verification_models[1])
                result3 = findMatch(image, source_embeddings[verification_models[2]["model"]], verification_models[2])
                end_time = time.perf_counter()
